...
//    });
    ...
    $('#txtObjeto').change(function() {
    	VerificarRescindir();
    });

    $('#fechaRescision').change(function() {
    	VerificarRescindir();
    });
...
//		var opt = $("input[type=radio]:checked");
	...
	$("#btnRescindir").click(function(e) {
		e.preventDefault();
		var id = $('#idcontrato').val();
		var idlote = $('#idlote').val();
		var objeto = $('#txtObjeto').val();
		var fecha = $('#fechaRescision').val();
		PreguntarescindirContrato(id, idlote, objeto, fecha);
	});
...
function rescindirContrato(id, idlote){// funcion que llamamos del archivo ajax/CategoriaAjax.php?op=delete linea 53

	$('#idcontrato').val(id);
	$('#idlote').val(idlote);
	$("#modalRescindir").modal("show");

/*
	swal({
	  title: "¿Esta Seguro de rescindir el Contrato seleccionado?",
//	  text: "Your will not be able to recover this imaginary file!",
	...
	function(){
		$.post("./ajax/ContratoAjax.php?op=rescindir", {id : id, idlote : idlote}, function(e){// llamamos la url de eliminar por post. y mandamos por parametro el id
...
function PreguntarescindirContrato(id, idlote, objeto, fecha){// funcion que llamamos del archivo ajax/CategoriaAjax.php?op=delete linea 53
	...
	function(){
		$.post("./ajax/ContratoAjax.php?op=rescindir", {id : id, idlote : idlote, objeto: objeto, fecha: fecha}, function(e){// llamamos la url de eliminar por post. y mandamos por parametro el id
...
*/
	...
	function VerificarRescindir() {
        var objeto = $("#txtObjeto").val();
        var fecha = $("#fechaRescision").val();
        var objetovacio = !objeto || 0 === objeto.length;
        var fechavacia = !fecha || 0 === fecha.length;
        if (objetovacio || fechavacia) {
			$('#btnRescindir').attr('disabled',true);
        } else {
			$('#btnRescindir').attr('disabled',false);
        }
	}
...
function cargarDataContrato(id, nrocontrato, idlote, idsector, idcementerio, observaciones, idadquiriente, adquiriente, idejecutivoventa, ejecutivoventa, fila, columna, tipoprecio, precio, cuotainicial, plazomeses, cuotamensual, incrementoplazo, fechacontrato, fechapago, nuevo, estado, fecharescindido, objeto) {
...
//        ComboLoteSel();
	    ...
	    $("#fechaResc").val(fecharescindido);

 		$("#txtObj").val(objeto);

 		if (estado=='rescindido') {
 			$("#txtFechaContrato").attr('readonly',true);
 			$("#cboNuevo").attr('readonly',true);
		    $("#txtAdquiriente").attr('readonly',true);
		    $("#txtEjecutivoVenta").attr('readonly',true);
		    $("#txtObservaciones").attr('readonly',true);
		    $("#btnBuscarEjecutivoVentaCto").attr('disabled',true);
		    $("#btnBuscarAdquirienteCto").attr('disabled',true);
		    $("#btnRegContrato").attr('disabled',true);
		    $("#tabprecios").hide();
		    $("#tabplan").hide();
 		} else {
 			$("#txtFechaContrato").attr('readonly',false);
 			$("#cboNuevo").attr('readonly',false);
		    $("#txtAdquiriente").attr('readonly',false);
		    $("#txtEjecutivoVenta").attr('readonly',false);
		    $("#txtObservaciones").attr('readonly',false);
		    $("#btnBuscarEjecutivoVentaCto").attr('disabled',false);
		    $("#btnBuscarAdquirienteCto").attr('disabled',false);
		    $("#btnRegContrato").attr('disabled',false);
		    $("#fechaResc").css("display", "none");
		    $("#txtObj").css("display", "none");;
		    $("#lblfechaResc").css("display", "none");
