			...
			if ($plazomeses > 0) {
				...
				if(empty($_POST["txtIdContrato2"])) {
					...
					if($objContrato->ModificarPrecios($idcontrato,$tipoprecio,$precio,$cuotainicial,$plazomeses,$cuotamensual,$incrementoplazo,$fechapago)){
						...
						if ($objContrato->RegistrarPlandePagos($precio,$incrementoplazo,$cuotainicial,$idcontrato,$plazomeses,$cuotamensual,$fechapago)) {
            ...
            $verReporte = true;
     		...
     		while ($reg = $query_Tipo->fetch_object()) {
     			...
     			$data[] = array(
                    ...
                    "11"=>
                    	($reg->estado=='activo' ? '<button class="btn btn-warning" data-toggle="tooltip" title="Editar" onclick="cargarDataContrato('.$reg->idcontrato.',\''.$reg->nrocontrato.'\','.$reg->idlote.','.$reg->idsector.','.$reg->idcementerio.',\''.$reg->observaciones.'\','.$reg->idadquiriente.',\''.$reg->adquiriente.'\','.$reg->idejecutivoventa.',\''.$reg->ejecutivoventa.'\',\''.$reg->fila.'\',\''.$reg->columna.'\',\''.$reg->tipoprecio.'\','.$reg->precio.','.$reg->cuotainicial.','.$reg->plazomeses.','.$reg->cuotamensual.','.$reg->incrementoplazo.',\''.$reg->fechacontrato.'\',\''.$reg->fechapago.'\',\''.$reg->nuevo.'\')"><i class="fa fa-pencil"></i> </button>&nbsp;' : '') .
     					...
     					($reg->estado=='activo' ? '<button class="btn btn-primary" data-toggle="tooltip" title=' . ($reg->plandepago=="S" ? '"Ver Plan de Pagos"': '"Debe generar Reporte Plan de Pagos"' ). ' ' . ($reg->plandepago=="S" ? '': 'disabled=""' ). ' onclick="consultarPlandePagos('. $reg->nrocontrato .')"><i class="fa fa-print"></i> </button>'  : ''));
