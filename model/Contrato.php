<?php
	...
	class Contrato {
		...
		public function TieneCuotaInicial($idcontrato) {
			global $conexion;

			$sql = "SELECT * FROM cuota WHERE (idcontrato = $idcontrato and nrocuota=0)";
		...
		public function RegistrarPlandePagos($precio,$incrementoplazo,$cuotainicial,$idcontrato,$plazomeses,$cuotamensual,$fechapago,$fechacontrato){

			global $conexion;

			$sql = "DELETE FROM cuota WHERE idcontrato = $idcontrato AND ( tipocuota='C' OR tipocuota='I')";

			$query = $conexion->query($sql);

			$nrocuota = 1;
			$fechacuota = new DateTime("$fechapago");
			$fechacontrato2 = new DateTime("$fechacontrato");
			...
			while ($nrocuota <= $plazomeses) {
	            ...
	            if ($nrocuota==1) {
		            $fechacontrato3 = date_format($fechacontrato2,"Y/m/d");

					$sql = "INSERT INTO cuota (idcontrato,nrocuota,fechalimite,monto,estado,tipocuota,acuenta,saldo)

								VALUES ($idcontrato,0,'$fechacontrato3',$cuotainicial,'P','C',0,$cuotainicial)";
...
//            $fechalimite2 = date_format($fechalimite,"Y-m-d");
		...
		public function Rescindir($idcontrato, $objeto, $fecha){

			global $conexion;

			$sql = "UPDATE contrato set estado='rescindido', objeto='$objeto', fecharescindido='$fecha' 
						WHERE idcontrato = $idcontrato";
...
*/
			...
			$sql = "select c.*, CONCAT(a.nombre,' ',a.apellidos) as adquiriente, CONCAT(a.tipodocumento,' ',a.numdocumento) as documento, a.celular as celular, a.telefono as telefono, CONCAT(e.nombre,' ',e.apellidos) as ejecutivoventa, m.razonsocial, s.nombre as sector, l.numero as nrolote, l.fila as fila, l.columna as columna, count(vc.nrocuota) as nrocuota, sum(vc.monto) as monto , sum(vc.saldo) as saldo from contrato c left join adquiriente a on c.idadquiriente = a.idadquiriente left join ejecutivoventa e on c.idejecutivoventa = e.idejecutivoventa left join cementerio m on c.idcementerio = m.idcementerio left join sector s on c.idsector = s.idsector left join lote l on c.idlote = l.idlote right join cuota vc on c.idcontrato=vc.idcontrato where c.estado='activo' and (vc.estado='V' or vc.estado='P') and vc.fechalimite<=" . $fecha . " group by c.idcontrato order by c.idcontrato desc ";
		...
		public function CrearPlandePagos($idcontrato,$tienecuotainicial) {
			...
			$querydatocontrato = $conexion->query($sqldatocontrato);
     		...
     		while ($reg = $querydatocontrato->fetch_object()) {
					...
					$x = 122;
					$bandera = false;
					if ($tienecuotainicial==true) {
						$saldo = ($reg->precio + $reg->incrementoplazo);
					} else {
			...
			$querydatocontrato = $conexion->query($sqldatocontrato);
     		...
     		while ($reg = $querydatocontrato->fetch_object()) {
					...
					$sql3 = "SELECT cobranza.idcobranza, cobranza.idcontrato, cobranza.fechacobranza, cobranza.nrorecibo, cobranzadetalle.nrocuota, cobranzadetalle.monto, cobranzadetalle.acuenta, cobranza.concepto, cobranza.tipopago, cobranza.observaciones, cobranza.nombre
					           FROM cobranza
					           INNER JOIN cobranzadetalle ON cobranza.idcobranza = cobranzadetalle.idcobranza 
					           WHERE cobranza.idcontrato = $idcontrato and cobranza.estado = 'activo' 
							   order by  cobranzadetalle.nrocuota;";

					$query3 = $conexion->query($sql3);

					$sqlsum = "SELECT sum(descuento) as descuento
					           FROM cobranza;";
					$querysum = $conexion->query($sqlsum);
					$Descuentos = 0;
					if ($rsum = $querysum->fetch_object()) {
						$Descuentos = $rsum->descuento;
					}
					//Títulos que llevará la cabecera
					/*$miCabecera = array(
						...
						$miCabecera = array(
							array('encabezado1'=>'', 'encabezado2'=>'NRO', 'encabezado3'=>'NRO', 'encabezado4'=>'TOTAL', 'encabezado5'=>'TOTAL', 'encabezado7'=>'OBJETO DEL', 'encabezado8'=>'SALDO','encabezado9'=>'TIPO DE', 'encabezado21'=>'FECHA', 'encabezado22'=>'RECIBO', 'encabezado23'=>'CUOTA', 'encabezado24'=>'CUOTA', 'encabezado25'=>'PAGO',  'encabezado27'=>'PAGO', 'encabezado28'=>'USD','encabezado29'=>'PAGO', 'len1' => 20, 'len2' => 20, 'len3' => 15, 'len4' => 20, 'len5' => 20,  'len7' => 34,'len8' => 20, 'len9' => 27)
						);
					...
					$saldo = ($reg->precio + $reg->incrementoplazo - $reg->cuotainicial);
		     		...
		     		while ($reg3 = $query3->fetch_object()) {
						 ...
						 $misDatos = array(array('fechacobranza' => $reg3->fechacobranza,'nrorecibo' => $reg3->nrorecibo, 'nrocuota' => $reg3->nrocuota, 'monto' => $reg3->monto, 'acuenta' => $reg3->acuenta, 'concepto' => $reg3->concepto, 'saldo' => number_format($saldo,2), 'tipopago' => $reg3->tipopago, 'len1' => 20, 'len2' => 20, 'len3' => 15, 'len4' => 20, 'len5' => 20, 'len7' => 34, 'len8' => 20,'len9' => 27) );
						$pdf->datosHorizontal($misDatos,$x,$bandera);
						$x = $x + 7;
						$bandera = !$bandera;
						if ($x>250) {
	     			...
	     			$misDatos = array(array('totalpagado' => $totalpagado,'saldo' => $saldo,'descuento' => $Descuentos) );
					$pdf->datosTotales($misDatos,$x);
